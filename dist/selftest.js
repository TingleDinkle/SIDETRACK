...
/* ----------------- simulation: movement / coupling / win / lose ----------------- */
{
    ...
    const lvl = (cols, rows, loco, wagons = []) => ({
        id: 't',
        world: 0,
        name: 't',
        grid: { cols, rows },
    ...
    const run = (g, level, maxT = 300) => {
        const s = new Simulation(g, level);
        let n = 0;
        while (s.status === 'running' && n++ < maxT)
            s.tick();
        return s;
    };
    ...
    const tunnel = (g, x, y, edge, pairId) => {
        const c = g.get(x, y);
        c.type = 'tunnel';
        c.fixed = true;
        c.pairId = pairId;
    ...
    const special = (g, x, y, type, mask, opts = {}) => {
        const c = g.get(x, y);
        c.type = type;
        c.fixed = true;
        c.mask = mask;
    ...
    // O) a coloured button fires once per train PASS, not once per car (no length parity)
    {
        const build = (nWag) => {
            ...
            for (let x = 1; x <= 8; x++)
                track(g, x, 0, EW);
            for (let i = 1; i <= nWag; i++)
                wag.push({ x: i, y: 0, number: i });
...
/* ----------------- level validation (editor edge cases) ----------------- */
{
    ...
    const errs = (l) => validateLevel(l).filter((i) => i.level === 'error').length;
    const with_ = (patch) => ({ ...good, ...patch });
    ...
    ok('validate: colourless gate -> error', errs(with_({ fixedTiles: [{ x: 4, y: 1, type: 'exit', heading: 'W' }, { x: 2, y: 1, type: 'gate', edges: ['W', 'E'] }] })) > 0);
    ok('validate: lone tunnel -> error', errs(with_({ fixedTiles: [{ x: 4, y: 1, type: 'exit', heading: 'W' }, { x: 2, y: 1, type: 'tunnel', edges: ['W'], pairId: 1 }] })) > 0);
    ok('validate: multi-edge tunnel -> error', errs(with_({
        fixedTiles: [
