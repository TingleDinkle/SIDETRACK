...
export class AssetManager {
    ...
    async load() {
        ...
        let atlas = null;
        if (manifest.atlas)
            atlas = await loadImage(this.base + manifest.atlas).catch(() => null);
    ...
    pattern(ctx, name) {
        ...
        if (s) {
            let source = s.img;
            const natW = s.img.naturalWidth ?? s.sw;
            const natH = s.img.naturalHeight ?? s.sh;
