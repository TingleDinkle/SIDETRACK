...
export const EdgeBit = { N: 1, E: 2, S: 4, W: 8 };
 ...
 * Edge-mask helpers
 * ------------------------------------------------------------------ */
export function hasEdge(mask, h) {
    return (mask & EdgeBit[h]) !== 0;
}
export function addEdge(mask, h) {
    return mask | EdgeBit[h];
}
export function removeEdge(mask, h) {
    return mask & ~EdgeBit[h];
}
/** The connected edges of a mask, in canonical N,E,S,W order. */
export function edgeList(mask) {
    const out = [];
 ...
 */
export function headingBetween(ax, ay, bx, by) {
    if (by === ay) {
        if (bx === ax + 1)
            return 'E';
        if (bx === ax - 1)
