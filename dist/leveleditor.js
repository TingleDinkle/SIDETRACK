...
export class LevelEditor {
    constructor(refs, onTest) {
        ...
        this.onMove = (e) => {
            ...
            if (this.tool === 'rock' || this.tool === 'erase')
                this.place(e.clientX, e.clientY);
        };
    ...
    buildToolButtons() {
        ...
        for (const t of TOOLS) {
            ...
            b.addEventListener('click', () => {
                this.tool = t.tool;
                for (const el of this.refs.tools.children)
                    el.classList.toggle('sel', el.dataset.tool === this.tool);
            });
    ...
    resizeGrid() {
        ...
        if (!inb(this.level.locomotive.x, this.level.locomotive.y))
            this.level.locomotive = { x: 0, y: 0, heading: 'E' };
        this.syncInputs();
        this.resize();
        this.draw();
    }
    place(clientX, clientY) {
        ...
        const heading = this.refs.heading.value;
        const color = this.refs.color.value;
        const edgesHV = this.refs.orient.value === 'V' ? ['N', 'S'] : ['W', 'E'];
        switch (this.tool) {
            case 'start':
            ...
            case 'switch':
                ...
                break;
            case 'wagon':
                ((_a = this.level).wagons ?? (_a.wagons = [])).push({ x, y, number: this.nextWagonNumber() });
                break;
            case 'mover':
                ((_b = this.level).movers ?? (_b.movers = [])).push({ x, y, heading });
                break;
        }
        this.draw();
    }
    /** Remove whatever occupies a cell. `keepStart` avoids nuking the loco when overwriting with a non-start tile elsewhere. */
    removeAt(x, y, _keepStart) {
    ...
    validate() {
        if (!this.level.fixedTiles.some((t) => t.type === 'exit'))
            return 'Add an Exit tile.';
        return null;
    }
    status(msg) {
...
function structuredCloneLevel(l) {
