...
export type TrackShape = 'none' | 'stub' | 'straight' | 'curve' | 'junction' | 'crossing';
...
export function classify(mask: EdgeMask): TrackShape {
 ...
 */
export function exitEdge(mask: EdgeMask, entryEdge: Heading, junctionActive = 0): Heading | null {
  if (!hasEdge(mask, entryEdge)) return null; // entered across a non-connected edge → derail
