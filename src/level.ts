...
import { Heading, TileType, addEdge } from './types.js';

export interface LevelTile {
  x: number;
  y: number;
  type: TileType;
  /** Fixed pre-laid track may be given as a raw mask or a list of edges. */
  mask?: number;
  edges?: Heading[];
  heading?: Heading;
  pairId?: number;
...
function edgesToMask(edges?: Heading[], mask?: number): number {
  if (typeof mask === 'number') return mask;
  let m = 0;
  if (edges) for (const e of edges) m = addEdge(m, e);
  return m;
}
...
export function buildGrid(level: Level): Grid {
  ...
  for (const t of level.fixedTiles) {
    ...
    if (t.heading) c.heading = t.heading;
    if (t.pairId !== undefined) c.pairId = t.pairId;
    if (t.color) c.color = t.color;
    if (t.open !== undefined) c.open = t.open;
    // An exit with only a heading still needs an edge to connect track to.
    if (c.type === 'exit' && c.mask === 0 && c.heading) c.mask = addEdge(0, c.heading);
  }
